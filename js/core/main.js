...
function downloadReport() {
    ...
    // 最终检查：如果按钮仍然被禁用或状态仍然锁定
    if (STATE.isDownloadLocked || (downloadBtn && (downloadBtn.disabled || downloadBtn.classList.contains('download-btn-locked')))) {
        console.log('❌ 下载仍然被锁定，显示支付弹窗');
        
        // 尝试最后修复
        if (downloadBtn) {
            console.log('🛠️ 执行最后修复尝试');
            downloadBtn.disabled = false;
            downloadBtn.classList.remove('download-btn-locked');
            downloadBtn.classList.remove('locked');
            ...
            // 再次检查
            if (!downloadBtn.disabled && !downloadBtn.classList.contains('download-btn-locked')) {
                console.log('✅ 最后修复成功，继续下载');
                // 继续执行下载逻辑
            } else {
                alert('请先解锁完整报告才能下载！');
                showPaymentModal();
...
重要提示：
1. 本报告仅供娱乐参考，不构成任何实际的人生建议
2. 命运掌握在自己手中，请理性对待分析结果
3. 如需进一步咨询，请联系客服微信：runzang888`;
    ...
    try {
        ...
        alert('下载失败，请稍后重试。错误信息：' + error.message);
    }
}
...
// ============ 【强制修复下载按钮函数】 ============
function forceUnlockDownloadButton() {
    console.log('🔧 强制解锁下载按钮...');
    ...
    console.log('✅ 下载按钮已强制解锁');
}

// ============ 【修改 handlePaymentSuccess 函数】 ============
function handlePaymentSuccess() {
    console.log('🔄 handlePaymentSuccess 被调用');
    ...
    // ✅ 关键修复：先调用原生函数，再调用强制修复
    if (typeof unlockDownloadButton === 'function') {
        console.log('调用原生 unlockDownloadButton');
        unlockDownloadButton();
    }
    
    // ✅ 强制修复确保解锁
    setTimeout(() => {
        forceUnlockDownloadButton();
    }, 100);
...
window.forceUnlockDownloadButton = forceUnlockDownloadButton; // 新增导出
