...
export async function showPaymentModal() {
    ...
    try {
        // 1. 获取可用支付方式
        const response = await fetch('https://runzang.top/api/payment/methods');
        const result = await response.json();
        
        if (!result.success || result.data.length === 0) {
            alert('当前没有可用的支付方式，请联系客服');
            return;
        }
        ...
        // 2. 显示支付选择弹窗
        showPaymentSelectionModal(serviceConfig, paymentMethods);
...
// 支付选择弹窗
function showPaymentSelectionModal(serviceConfig, paymentMethods) {
    // 创建支付选择弹窗
    const modalHtml = `
    <div class="payment-selection-modal" id="payment-selection-modal">
        <div class="payment-modal-content">
            <div class="payment-modal-header">
                <h3>选择支付方式</h3>
                <span class="close-btn" id="close-payment-selection">&times;</span>
            </div>
            
            <div class="order-info">
                <div class="info-item">
                    <span>服务类型：</span>
                    <strong>${STATE.currentService}</strong>
                </div>
                <div class="info-item">
                    <span>支付金额：</span>
                    <strong class="price">¥${serviceConfig.price}</strong>
                </div>
            </div>
            
            <div class="payment-methods-list">
                ${paymentMethods.map(method => `
                <div class="payment-method-item" data-method="${method.value}">
                    ...
                    <div class="method-select">
                        <button class="select-btn" data-method="${method.value}">选择</button>
                    </div>
                </div>
                `).join('')}
            </div>
            ...
            <div class="payment-tips">
                <p>⚠️ 支付成功后请勿关闭页面，系统会自动跳转</p>
                <p>如有问题请联系客服微信：runzang888</p>
            </div>
        </div>
    </div>
    `;
    ...
    // 绑定事件
    setupPaymentSelectionEvents(modal, serviceConfig);
}

function setupPaymentSelectionEvents(modal, serviceConfig) {
    // 关闭按钮
    modal.querySelector('#close-payment-selection').addEventListener('click', () => {
        modal.remove();
    ...
    modal.querySelectorAll('.select-btn').forEach(btn => {
        btn.addEventListener('click', async function() {
            ...
            await createPaymentOrder(paymentMethod, serviceConfig);
            modal.remove();
        });
    });
...
async function createPaymentOrder(paymentMethod, serviceConfig) {
    try {
        console.log('创建支付订单，支付方式:', paymentMethod);
        
        // 显示支付确认弹窗
        const confirmed = await showPaymentConfirmModal(paymentMethod, serviceConfig);
...
// 新支付系统返回的数据格式
const { paymentUrl, outTradeNo, amount, subject, qrCodeUrl } = result.data;

// 保存订单ID
STATE.currentOrderId = outTradeNo;

// 保存分析结果到localStorage（防止丢失）
saveAnalysisData();

console.log('支付宝支付URL:', paymentUrl);
...
function showPaymentConfirmModal(paymentMethod, serviceConfig) {
    return new Promise((resolve) => {
        const modalHtml = `
        <div class="payment-confirm-modal" id="payment-confirm-modal">
            <div class="confirm-modal-content">
                ...
                <div class="confirm-body">
                    <div class="payment-method-display">
                        <span class="method-icon ${paymentMethod === 'alipay' ? 'alipay-icon' : 'wechat-icon'}"></span>
                        <span>${paymentMethod === 'alipay' ? '支付宝支付' : '微信支付'}</span>
                    </div>
                    
                    <div class="amount-display">
                        <span>支付金额：</span>
                        <strong class="price-large">¥${serviceConfig.price}</strong>
                    </div>
                    
                    <div class="service-info">
                        <p>服务项目：${STATE.currentService}</p>
                        <p>包含内容：${serviceConfig.lockedItems.length}项详细分析</p>
                    </div>
                </div>
                
                <div class="confirm-footer">
                    <button class="btn-cancel" id="cancel-payment">取消</button>
                    <button class="btn-confirm" id="confirm-payment">确认支付</button>
                </div>
            </div>
        </div>
        `;
        ...
        // 绑定事件
        modal.querySelector('#close-confirm-modal').addEventListener('click', () => {
            modal.remove();
            resolve(false);
        });
