...
export async function showPaymentModal() {
  ...
  try {
    ...
    const sign = await generateRequestSignature(STATE.currentService, timestamp);
    
    const response = await fetch(`${PAYMENT_CONFIG.GATEWAY_URL}/create`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Request-Source': 'github_pages'
      },
      body: JSON.stringify({
    ...
    if (!response.ok) {
      throw new Error(`支付网关请求失败: ${response.status}`);
    }
    ...
    const { paymentUrl, outTradeNo, amount, serviceType } = result.data;
    
    // 2. 保存订单信息到状态
    STATE.currentOrder = {
      serviceType: STATE.currentService,
      displayName: serviceType, // 合规化名称
      price: amount,
      orderId: outTradeNo,
    ...
    // 3. 更新支付弹窗显示真实信息
    UI.paymentServiceType().textContent = serviceType;
    UI.paymentAmount().textContent = '¥' + amount;
    UI.paymentOrderId().textContent = outTradeNo;
    
    // 4. 隐藏模拟的二维码，显示支付宝跳转按钮
    document.querySelector('.qr-codes').style.display = 'none';
    
    const redirectBtn = document.createElement('button');
    redirectBtn.id = 'alipay-redirect-btn';
    redirectBtn.className = 'dynamic-pulse-btn';
    ...
    redirectBtn.onclick = () => {
      // 记录点击时间，用于分析
      console.log(`用户跳转支付宝，订单: ${outTradeNo}`);
      // 新窗口打开支付页面
    ...
    orderInfo.parentNode.insertBefore(redirectBtn, orderInfo.nextSibling);
