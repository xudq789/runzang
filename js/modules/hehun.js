...
请先进行八字排盘。大运顺逆规则：阳男阴女顺行、阴男阳女逆行。起运岁数计算规则：1、如果大运顺行，先确认下一个节气年月日时分，相差天数 = (下一个节气时间 - 出生时间)，起运岁数 = 相差天数 ÷ 3天/岁；2、如果大运逆行，先确认上一个节气年月日时分，相差天数 = (出生时间 - 上一个节气时间)，起运岁数 = 相差天数 ÷ 3天/岁)。然后用以下格式输出结果：
